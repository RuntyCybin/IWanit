...
<template>
  <main class="min-h-screen bg-slate-100 flex items-center justify-center p-4">
    <div class="w-full max-w-md bg-white rounded-xl shadow-sm border border-slate-200 p-6">
      ...
      <div v-if="token" class="mt-4 rounded-lg bg-emerald-50 px-3 py-2">
        <p class="text-sm font-medium text-emerald-800">Token recibido</p>
        <p class="mt-1 break-all font-mono text-xs text-emerald-700">{{ token }}</p>
      </div>
    </div>
  </main>
</template>
